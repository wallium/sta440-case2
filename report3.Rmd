...
# Full vs reduced
m3 <- glm(y ~ 0 + X1 + X2 + X3 + X4 + X5 + X6 + X7 + male + black + hisp, data=d2, family="binomial")
anova(m2,m3)
...
hazard.logodds.2 <- predict(m2, newdata = dt_2)
hazard.2 <- exp(hazard.logodds.2)/(1+exp(hazard.logodds.2))

plot(1:length(hazard.1), hazard.1, type='l', ylim=range(c(0,1)), col = c('blue'),
     xlab = "Time Bin",
     ylab = "Hazard")

lines(1:length(hazard.2), hazard.2, type='l', col = c('red'))
legend("topright", c("no symptom", "4 symptoms"), lty=c(1,1), lwd=c(2.5,2.5),col=c("blue", "red"))
