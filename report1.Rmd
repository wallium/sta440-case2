...
kellydat$sympton = rep(0, length(kellydat$sn1))
for (i in 1:length(kellydat$nctdel)){
  if (kellydat$sn1[i] == 1){
    kellydat$sympton[i] = 1
  }
  if (kellydat$sn2[i] == 1){
    kellydat$sympton[i] = 2
  } 
  if (kellydat$sn3[i] == 1){
    kellydat$sympton[i] = 3
  }
  if (kellydat$all4[i] == 1){
    kellydat$sympton[i] = 4
  }
}
```
...
# Correlation between Assessment Time and Number of Symptons
```{r, echo=FALSE, warning=FALSE, message=FALSE}
ggplot(data = kellydat) + 
  geom_smooth(mapping = aes(x = sympton, y = nctdel))
```
To further explore the possible correlation between the nctdel time and number of symtons, we plot a smooth fitted line of nctdel time vs sympton. We can observe that the nctdel time is likely to be shorter for patients with more symptoms.
